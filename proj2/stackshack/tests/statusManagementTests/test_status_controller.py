...
class TestStatusController:
    ...
    def test_update_order_status_pending_to_preparing(self, app, pending_order):
        ...
        with app.app_context():
            success, msg, updated_order = StatusController.update_order_status(pending_order, 'Preparing')
    ...
    def test_update_order_status_nonexistent_order(self, app):
        ...
        with app.app_context():
            success, msg, order = StatusController.update_order_status(99999, 'Preparing')
    ...
    def test_cancel_pending_order(self, app, test_customer_user, pending_order):
        ...
        with app.app_context():
            success, msg, cancelled = StatusController.cancel_order(pending_order, test_customer_user)
    ...
    def test_cancel_preparing_order(self, app, test_customer_user, preparing_order):
        ...
        with app.app_context():
            success, msg, cancelled = StatusController.cancel_order(preparing_order, test_customer_user)
    ...
    def test_cancel_ready_order(self, app, test_customer_user, ready_order):
        ...
        with app.app_context():
            success, msg, cancelled = StatusController.cancel_order(ready_order, test_customer_user)
            
            assert success is True
            assert cancelled.status == 'Cancelled'
    
    def test_cannot_cancel_delivered_order(self, app, test_customer_user, delivered_order):
        """Test cannot cancel a delivered order."""
        with app.app_context():
            success, msg, order = StatusController.cancel_order(delivered_order, test_customer_user)
            
            assert success is False
            assert 'delivered' in msg.lower()
    
    def test_cannot_cancel_already_cancelled_order(self, app, test_customer_user, cancelled_order):
        """Test cannot cancel an already cancelled order."""
        with app.app_context():
            success, msg, order = StatusController.cancel_order(cancelled_order, test_customer_user)
            
            assert success is False
            assert 'cancelled' in msg.lower()
    
    def test_cancel_order_wrong_user(self, app, test_customer_user, test_staff_user, pending_order):
        """Test cannot cancel another user's order."""
        with app.app_context():
            success, msg, order = StatusController.cancel_order(pending_order, test_staff_user)
    ...
    def test_cancel_nonexistent_order(self, app, test_customer_user):
        ...
        with app.app_context():
            success, msg, order = StatusController.cancel_order(99999, test_customer_user)
    ...
    def test_get_order_by_id_success(self, app, test_customer_user, pending_order):
        ...
        with app.app_context():
            success, msg, order = StatusController.get_order_by_id(pending_order, test_customer_user)
    ...
    def test_get_order_by_id_wrong_user(self, app, test_staff_user, pending_order):
        ...
        with app.app_context():
            success, msg, order = StatusController.get_order_by_id(pending_order, test_staff_user)
    ...
    def test_get_order_by_id_nonexistent(self, app, test_customer_user):
        ...
        with app.app_context():
            success, msg, order = StatusController.get_order_by_id(99999, test_customer_user)
    ...
    def test_get_all_orders_for_staff_ordering(self, app, multiple_orders_various_statuses):
